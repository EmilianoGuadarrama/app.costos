...
<div class="dash-form-view">
    ...
    <div class="form-panel">
        <div class="header-section"><h1>Editar Reporte</h1></div>
        <form action="{{ route('reportes.update', $reporte) }}" method="POST">
            ...
            <div class="form-group">
                ...
                <select id="presupuesto_id" name="presupuesto_id" class="form-select" required>
                    ...
                    @foreach($presupuestos as $p)
                        <option value="{{ $p->id }}" {{ old('presupuesto_id', $reporte->presupuesto_id) == $p->id ? 'selected' : '' }}>{{ $p->nombre }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input type="text" id="nombre" name="nombre" class="form-control" value="{{ old('nombre', $reporte->nombre) }}" required maxlength="150">
                @error('nombre') <span class="text-danger">{{ $message }}</span> @enderror
            </div>
            <div class="form-group">
                <label for="tipo_salida">Tipo de Salida *</label>
                <select id="tipo_salida" name="tipo_salida" class="form-select" required>
                    <option value="pdf" {{ old('tipo_salida', $reporte->tipo_salida) == 'pdf' ? 'selected' : '' }}>PDF</option>
                    <option value="excel" {{ old('tipo_salida', $reporte->tipo_salida) == 'excel' ? 'selected' : '' }}>Excel</option>
                    <option value="vista" {{ old('tipo_salida', $reporte->tipo_salida) == 'vista' ? 'selected' : '' }}>Vista en Pantalla</option>
                </select>
                @error('tipo_salida') <span class="text-danger">{{ $message }}</span> @enderror
            </div>
            <div class="form-group">
                <label for="fecha_generacion">Fecha de Generación *</label>
                <input type="datetime-local" id="fecha_generacion" name="fecha_generacion" class="form-control" value="{{ old('fecha_generacion', \Carbon\Carbon::parse($reporte->fecha_generacion)->format('Y-m-d\TH:i')) }}" required>
                @error('fecha_generacion') <span class="text-danger">{{ $message }}</span> @enderror
            </div>
            <div class="form-group">
                <label for="ruta_archivo">Ruta del Archivo</label>
                <input type="text" id="ruta_archivo" name="ruta_archivo" class="form-control" value="{{ old('ruta_archivo', $reporte->ruta_archivo) }}" maxlength="255">
