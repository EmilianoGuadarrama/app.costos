@extends('layout')
@section('title','Detalle Categoría de Egreso')
@section('content')
...
<div class="dash-form-view">
    <a href="{{ route('categorias_egreso.index') }}" class="btn-back"><i class="bi bi-arrow-left"></i> Volver</a>
    <div class="form-panel">
        <div class="header-section">
            <h1>{{ $categorias_egreso->nombre }}</h1>
            <a href="{{ route('categorias_egreso.edit', $categorias_egreso) }}" class="btn btn-sm btn-outline-dark"><i class="bi bi-pencil"></i> Editar</a>
        </div>
        <div class="detail-row"><div class="detail-label">Nombre</div><div class="detail-value">{{ $categorias_egreso->nombre }}</div></div>
        <div class="detail-row"><div class="detail-label">Descripción</div><div class="detail-value">{{ $categorias_egreso->descripcion ?? '—' }}</div></div>
        <div class="detail-row"><div class="detail-label">Creado</div><div class="detail-value">{{ $categorias_egreso->created_at?->format('d/m/Y H:i') ?? '—' }}</div></div>
    </div>
</div>
@endsection
