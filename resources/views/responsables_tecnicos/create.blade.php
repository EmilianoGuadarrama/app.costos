@extends('layout')
@section('title','Nuevo Responsable Técnico')
@section('content')
...
<div class="dash-form-view">
    ...
    <div class="form-panel">
        <div class="header-section"><h1>Nuevo Responsable Técnico</h1></div>
        <form action="{{ route('responsables_tecnicos.store') }}" method="POST">
            @csrf
            ...
            <div class="form-group">
                ...
                <input type="text" id="cargo" name="cargo" class="form-control" value="{{ old('cargo') }}" required maxlength="120">
                @error('cargo') <span class="text-danger">{{ $message }}</span> @enderror
            </div>
            <div class="form-group">
                <label for="firma_path">Firma (Ruta/URL)</label>
                <input type="text" id="firma_path" name="firma_path" class="form-control" value="{{ old('firma_path') }}" maxlength="255">
                @error('firma_path') <span class="text-danger">{{ $message }}</span> @enderror
            </div>
            <button type="submit" class="btn-submit">Guardar Responsable</button>
        </form>
    </div>
</div>
@endsection
