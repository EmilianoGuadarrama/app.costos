...
<div class="dash-index-view">
    <div class="index-panel">
        ...
        <div class="header-section">
            <div><h1>Estados de Proyecto</h1><p>Catálogo de estados disponibles para proyectos.</p></div>
            <div class="header-actions"><a href="{{ route('estados_proyecto.create') }}" class="btn-add-new"><i class="bi bi-plus-circle me-1"></i> Nuevo Estado</a></div>
        </div>
        <div class="table-responsive">
            <table class="project-table">
                ...
                @forelse($estados as $estado)
                    @php $estadoId = $estado->id; @endphp
                    <tr class="project-row">
                        <td>{{ $estado->id }}</td>
                        <td><strong>{{ $estado->nombre }}</strong></td>
                        <td class="action-cell">
                            <button type="button" class="btn-icon-action" title="Ver" data-bs-toggle="modal" data-bs-target="#verEstadoModal{{ $estadoId }}"><i class="bi bi-eye"></i></button>
                            <button type="button" class="btn-icon-action" title="Eliminar" data-bs-toggle="modal" data-bs-target="#eliminarEstadoModal{{ $estadoId }}"><i class="bi bi-trash3"></i></button>
                        </td>
                    </tr>
                    <div class="modal fade" id="verEstadoModal{{ $estadoId }}" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered"><div class="modal-content">
                            <div class="modal-header"><h5 class="modal-title">{{ $estado->nombre }}</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button></div>
                            <div class="modal-body"><div class="detail-box" style="margin:0;"><h6>Información</h6><p><strong>ID:</strong> {{ $estado->id }}</p><p><strong>Nombre:</strong> {{ $estado->nombre }}</p></div></div>
                            <div class="modal-footer"><button type="button" class="btn-modal-light" data-bs-dismiss="modal">Cerrar</button><a href="{{ route('estados_proyecto.edit', $estadoId) }}" class="btn-modal-dark">Editar</a></div>
                        </div></div>
                    </div>
                    <div class="modal fade" id="eliminarEstadoModal{{ $estadoId }}" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered"><div class="modal-content">
                            <div class="modal-header"><h5 class="modal-title">Eliminar estado</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button></div>
                            <div class="modal-body">¿Deseas eliminar el estado <strong>{{ $estado->nombre }}</strong>?<br><br>Esta acción no se puede deshacer.</div>
                            <div class="modal-footer"><button type="button" class="btn-modal-light" data-bs-dismiss="modal">Cancelar</button><form action="{{ route('estados_proyecto.destroy', $estadoId) }}" method="POST">@csrf @method('DELETE')<button type="submit" class="btn-modal-danger">Eliminar</button></form></div>
                        </div></div>
                    </div>
                @empty
