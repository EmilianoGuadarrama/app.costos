@extends('layout')
@section('title','Detalle Maquinaria')
@section('content')
...
<div class="dash-form-view">
    ...
    <div class="form-panel">
        <div class="header-section">
            <h1>{{ $maquinaria->equipo }}</h1>
            <a href="{{ route('maquinaria_equipo.edit', $maquinaria) }}" class="btn btn-sm btn-outline-dark"><i class="bi bi-pencil"></i> Editar</a>
        </div>
        <div class="detail-row"><div class="detail-label">Clave</div><div class="detail-value">{{ $maquinaria->clave }}</div></div>
        <div class="detail-row"><div class="detail-label">Equipo</div><div class="detail-value">{{ $maquinaria->equipo }}</div></div>
        <div class="detail-row"><div class="detail-label">Unidad</div><div class="detail-value">{{ $maquinaria->unidadMedida->nombre ?? 'N/A' }} ({{ $maquinaria->unidadMedida->abreviatura ?? '' }})</div></div>
        <div class="detail-row"><div class="detail-label">Costo por Hora</div><div class="detail-value">${{ number_format($maquinaria->costo_por_hora, 2) }}</div></div>
        <div class="detail-row"><div class="detail-label">Creado</div><div class="detail-value">{{ $maquinaria->created_at?->format('d/m/Y H:i') ?? '—' }}</div></div>
    </div>
</div>
@endsection
