...
<div class="dash-index-view">
    <div class="index-panel">
        <div class="header-section">
            <div><h1>Ingresos</h1></div>
            <div class="header-actions">
                <a href="{{ route('ingresos.create') }}" class="btn-add-new"><i class="bi bi-plus-circle me-1"></i> Nuevo Ingreso</a>
            </div>
        </div>
                ...
                @forelse($ingresos as $ingreso)
                    <tr class="project-row">
                        ...
                        <td class="action-cell">
                            <a href="{{ route('ingresos.show', $ingreso) }}" class="btn-icon-action"><i class="bi bi-eye"></i></a>
                            <a href="{{ route('ingresos.edit', $ingreso) }}" class="btn-icon-action"><i class="bi bi-pencil-square"></i></a>
                            <form action="{{ route('ingresos.destroy', $ingreso) }}" method="POST" style="display:inline;" onsubmit="return confirm('¿Eliminar ingreso?');">
                                @csrf @method('DELETE')
