...
<div class="dash-index-view">
    <div class="index-panel">
        <div class="header-section">
            <div><h1>Categorías de Egreso</h1></div>
            <div class="header-actions">
                <a href="{{ route('categorias_egreso.create') }}" class="btn-add-new"><i class="bi bi-plus-circle me-1"></i> Nueva Categoría</a>
            </div>
        </div>
                ...
                @forelse($categorias as $categoria)
                    <tr class="project-row">
                        ...
                        <td class="action-cell">
                            <a href="{{ route('categorias_egreso.show', $categoria) }}" class="btn-icon-action"><i class="bi bi-eye"></i></a>
                            <a href="{{ route('categorias_egreso.edit', $categoria) }}" class="btn-icon-action"><i class="bi bi-pencil-square"></i></a>
                            <form action="{{ route('categorias_egreso.destroy', $categoria) }}" method="POST" style="display:inline;" onsubmit="return confirm('¿Eliminar categoría?');">
                                @csrf @method('DELETE')
