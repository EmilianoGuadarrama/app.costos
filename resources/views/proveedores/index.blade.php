    ...
    <div class="index-panel">
        <div class="header-section">
            ...
            <div class="header-actions">
                <a href="{{ route('proveedores.create') }}" class="btn-add-new"><i class="bi bi-plus-circle me-1"></i> Nuevo Proveedor</a>
            </div>
        </div>
            ...
            <table class="project-table">
                ...
                @forelse($proveedores as $proveedore)
                    <tr class="project-row">
                        ...
                        <td>
                            @if($proveedore->tipo)
                                <span class="badge-soft">{{ $proveedore->tipo }}</span>
                            @else
                                —
                            @endif
                        </td>
                        ...
                        <td class="action-cell">
                            <a href="{{ route('proveedores.show', $proveedore) }}" class="btn-icon-action"><i class="bi bi-eye"></i></a>
                            <a href="{{ route('proveedores.edit', $proveedore) }}" class="btn-icon-action"><i class="bi bi-pencil-square"></i></a>
                            <form action="{{ route('proveedores.destroy', $proveedore) }}" method="POST" style="display:inline;" onsubmit="return confirm('¿Eliminar proveedor?');">
                                @csrf @method('DELETE')
