...
<style>
    ...
    .index-panel{ background:#fff; padding:40px; border-radius:12px; box-shadow:0 4px 10px rgba(0,0,0,.05); max-width:1100px; margin:0 auto; }
    ...
    <div class="index-panel">
        <div class="header-section">
            ...
            <div class="header-actions">
                <a href="{{ route('compras.create') }}" class="btn-add-new"><i class="bi bi-plus-circle me-1"></i> Nueva Compra</a>
            </div>
        </div>
                ...
                @forelse($compras as $compra)
                    <tr class="project-row">
                        ...
                        <td class="action-cell">
                            <a href="{{ route('compras.show', $compra) }}" class="btn-icon-action"><i class="bi bi-eye"></i></a>
                            <a href="{{ route('compras.edit', $compra) }}" class="btn-icon-action"><i class="bi bi-pencil-square"></i></a>
                            <form action="{{ route('compras.destroy', $compra) }}" method="POST" style="display:inline;" onsubmit="return confirm('¿Eliminar compra?');">
                                @csrf @method('DELETE')
