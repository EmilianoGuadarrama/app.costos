...
<style>
    ...
    @media (max-width: 992px){ .index-panel{ padding:24px; } .project-table{ min-width:980px; } .detail-grid{ grid-template-columns:1fr; } }
</style>
...
<div class="dash-index-view">
    <div class="index-panel">
        ...
        <div class="header-section">
            <div><h1>Indirectos</h1><p>Listado general de costos indirectos y porcentaje aplicado.</p></div>
            <div class="header-actions">
                <button class="btn-filter" type="button"><i class="bi bi-funnel me-1"></i> Filtrar</button>
                <a href="{{ Route::has('indirectos.create') ? route('indirectos.create') : '#' }}" class="btn-add-new"><i class="bi bi-plus-circle me-1"></i> Nuevo Indirecto</a>
            </div>
        </div>
        <div class="table-responsive">
            <table class="project-table">
                <thead><tr><th style="width:35%;">Detalle del Indirecto</th><th>Cálculo</th><th>Tipo</th><th style="text-align:right;">Acciones</th></tr></thead>
                <tbody>
                @forelse(($indirectos ?? []) as $indirecto)
                    @php $indId = $indirecto->id ?? $indirecto->id_indirecto ?? 1; @endphp
                    <tr class="project-row">
                        <td>
                            <div class="title-main">{{ $indirecto->concepto ?? 'Indirecto sin concepto' }}<span class="badge-dark-mini">{{ $indirecto->clave ?? 'IND' }}</span></div>
                            <div class="desc-text">Costo indirecto aplicado al análisis o presupuesto general.</div>
                        </td>
                        <td><div class="info-stack"><div><strong>Porcentaje:</strong> {{ $indirecto->porcentaje ?? '0' }}%</div><div><strong>Importe:</strong> {{ $indirecto->importe ?? '0.00' }}</div></div></td>
                        <td><span class="badge-soft">Indirecto</span></td>
                        <td class="action-cell">
                            <button type="button" class="btn-icon-action" title="Ver" data-bs-toggle="modal" data-bs-target="#verIndModal{{ $indId }}"><i class="bi bi-eye"></i></button>
                            <button type="button" class="btn-icon-action" title="Eliminar" data-bs-toggle="modal" data-bs-target="#eliminarIndModal{{ $indId }}"><i class="bi bi-trash3"></i></button>
                        </td>
                    </tr>
                    <div class="modal fade" id="verIndModal{{ $indId }}" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog modal-lg modal-dialog-centered"><div class="modal-content">
                            <div class="modal-header"><h5 class="modal-title">{{ $indirecto->concepto ?? 'Indirecto' }}</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button></div>
                            <div class="modal-body"><div class="detail-grid">
                                <div class="detail-box"><h6>Información</h6><p><strong>Concepto:</strong> {{ $indirecto->concepto ?? '—' }}</p><p><strong>Clave:</strong> {{ $indirecto->clave ?? '—' }}</p></div>
                                <div class="detail-box"><h6>Cálculo</h6><p><strong>Porcentaje:</strong> {{ $indirecto->porcentaje ?? '0' }}%</p><p><strong>Importe:</strong> {{ $indirecto->importe ?? '0.00' }}</p></div>
                            </div></div>
                            <div class="modal-footer"><button type="button" class="btn-modal-light" data-bs-dismiss="modal">Cerrar</button><a href="{{ Route::has('indirectos.edit') ? route('indirectos.edit', $indId) : '#' }}" class="btn-modal-dark">Editar</a></div>
                        </div></div>
                    </div>
                    <div class="modal fade" id="eliminarIndModal{{ $indId }}" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered"><div class="modal-content">
                            <div class="modal-header"><h5 class="modal-title">Eliminar indirecto</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button></div>
                            <div class="modal-body">¿Deseas eliminar el costo indirecto <strong>{{ $indirecto->concepto ?? '' }}</strong>?<br><br>Esta acción no se puede deshacer.</div>
                            <div class="modal-footer"><button type="button" class="btn-modal-light" data-bs-dismiss="modal">Cancelar</button>
                                @if(Route::has('indirectos.destroy'))
                                <form action="{{ route('indirectos.destroy', $indId) }}" method="POST">@csrf @method('DELETE')<button type="submit" class="btn-modal-danger">Eliminar</button></form>
                                @else
                ...
                @empty
                    <tr class="project-row"><td colspan="4" class="empty-state">No hay costos indirectos registrados.</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
