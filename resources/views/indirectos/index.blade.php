...
@section('content')
    ...
    <style>
        ...
        @media (max-width: 992px){ .index-panel{ padding:24px; } .project-table{ min-width:980px; } }
    </style>
    ...
    <div class="dash-index-view">
        <div class="index-panel">
            <div class="header-section">
                <div>
                    <h1>Indirectos</h1>
                    <p>Listado general de costos indirectos y porcentaje aplicado.</p>
                </div>
                ...
                <div class="header-actions">
                    ...
                    <a href="{{ Route::has('indirectos.create') ? route('indirectos.create') : '#' }}" class="btn-add-new">
            ...
            <div class="table-responsive">
                <table class="project-table">
                    <thead>
                    <tr>
                        <th style="width:35%;">Detalle del Indirecto</th>
                        <th>Cálculo</th>
                        <th>Tipo</th>
                    ...
                    @forelse(($indirectos ?? []) as $indirecto)
                        @php $indirectoId = $indirecto->id ?? $indirecto->id_indirecto ?? 1; @endphp
                        <tr class="project-row">
                            <td>
                                <div class="title-main">
                                    {{ $indirecto->concepto ?? 'Indirecto sin concepto' }}
                                    <span class="badge-dark-mini">{{ $indirecto->clave ?? 'IND' }}</span>
                                </div>
                                <div class="desc-text">Costo indirecto aplicado al análisis o presupuesto general.</div>
                            </td>
                            <td>
                                <div class="info-stack">
                                    <div><strong>Porcentaje:</strong> {{ $indirecto->porcentaje ?? '0' }}%</div>
                                    <div><strong>Importe:</strong> {{ $indirecto->importe ?? '0.00' }}</div>
                                </div>
                            </td>
                            <td>
                                <span class="badge-soft">Indirecto</span>
                            </td>
                            <td class="action-cell">
                                <a href="{{ Route::has('indirectos.show') ? route('indirectos.show', $indirectoId) : '#' }}" class="btn-icon-action" title="Ver">
                                    <i class="bi bi-eye"></i>
                                </a>
                                <a href="{{ Route::has('indirectos.edit') ? route('indirectos.edit', $indirectoId) : '#' }}" class="btn-icon-action" title="Editar">
                                    <i class="bi bi-pencil-square"></i>
                                </a>
                                @if(Route::has('indirectos.destroy'))
                                    <form action="{{ route('indirectos.destroy', $indirectoId) }}" method="POST" style="display:inline;" onsubmit="return confirm('¿Eliminar este costo indirecto?');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr class="project-row">
                            <td colspan="4" class="empty-state">No hay costos indirectos registrados.</td>
                        </tr>
                    @endforelse
                    </tbody>
                </table>
