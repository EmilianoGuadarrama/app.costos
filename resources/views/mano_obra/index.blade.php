...
<style>
    ...
    .work-table{ width:100%; border-collapse:separate; border-spacing:0 12px; }
    .work-table thead th{ text-align:left; color:#888; font-size:.75rem; letter-spacing:2px; text-transform:uppercase; padding:0 20px 10px; font-family:Arial,sans-serif; }
    .work-row{ background:#fff; outline:1px solid #eee; transition:all .3s ease; }
    .work-row:hover{ transform:translateY(-2px); box-shadow:0 5px 15px rgba(0,0,0,.05); }
    .work-row td{ padding:15px 20px; vertical-align:middle; }
    ...
    .amount-badge{ display:inline-flex; align-items:center; padding:6px 12px; border-radius:12px; background:#eee; color:#111; font-family:Arial,sans-serif; font-size:.82rem; font-weight:700; }
    ...
    @media (max-width: 992px){ .index-panel{ padding:24px; } .work-table{ min-width:980px; } .detail-grid{ grid-template-columns:1fr; } }
</style>
...
<div class="dash-index-view">
    <div class="index-panel">
        ...
        <div class="header-section">
            <div><h1>Mano de Obra</h1><p>Listado general de categorías, unidades y salarios de mano de obra.</p></div>
            <div class="header-actions">
                <button class="btn-filter" type="button"><i class="bi bi-funnel me-1"></i> Filtrar</button>
                <a href="{{ route('mano_obra.create') }}" class="btn-add-new"><i class="bi bi-plus-circle me-1"></i> Nueva Mano de Obra</a>
            </div>
        </div>

        <div class="table-responsive">
            <table class="work-table">
                <thead><tr><th style="width:40%;">Detalle de Mano de Obra</th><th>Pago</th><th>Unidad</th><th style="text-align:right;">Acciones</th></tr></thead>
                <tbody>
                @forelse(($manoObras ?? []) as $mano)
                    ...
                    <tr class="work-row">
                        ...
                        <td class="action-cell">
                            ...
                            <button type="button" class="btn-icon-action" title="Eliminar" data-bs-toggle="modal" data-bs-target="#eliminarManoModal{{ $manoId }}"><i class="bi bi-trash3"></i></button>
                        </td>
                    </tr>
                ...
                @empty
                    <tr class="work-row"><td colspan="4" class="empty-state">No hay registros de mano de obra disponibles.</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
