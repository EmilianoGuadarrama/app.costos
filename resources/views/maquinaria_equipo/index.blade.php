...
<div class="dash-index-view">
    <div class="index-panel">
        ...
        <div class="header-section">
            <div><h1>Maquinaria y Equipo</h1><p>Catálogo de maquinaria y equipo con costos horarios.</p></div>
            <div class="header-actions">
                <button class="btn-filter" type="button"><i class="bi bi-funnel me-1"></i> Filtrar</button>
                <a href="{{ route('maquinaria_equipo.create') }}" class="btn-add-new"><i class="bi bi-plus-circle me-1"></i> Nuevo Equipo</a>
            </div>
        </div>

        <div class="table-responsive">
            <table class="project-table">
                <thead><tr><th style="width:40%;">Detalle</th><th>Costo</th><th>Unidad</th><th style="text-align:right;">Acciones</th></tr></thead>
                <tbody>
                @forelse(($maquinarias ?? []) as $maquinaria)
                    @php $maqId = $maquinaria->id; @endphp
                    <tr class="project-row">
                        <td>
                            <div class="title-main">{{ $maquinaria->descripcion ?? 'Sin descripción' }}<span class="badge-dark-mini">{{ $maquinaria->clave ?? 'EQ' }}</span></div>
                            <div class="desc-text">Registro de maquinaria o equipo para análisis de P.U.</div>
                        </td>
                        <td><span class="badge-soft">${{ number_format((float) ($maquinaria->costo_horario ?? 0), 2) }} /hr</span></td>
                        <td><div class="info-stack">{{ optional($maquinaria->unidadMedida)->nombre ?? 'N/D' }}</div></td>
                        <td class="action-cell">
                            <button type="button" class="btn-icon-action" title="Ver" data-bs-toggle="modal" data-bs-target="#verMaqModal{{ $maqId }}"><i class="bi bi-eye"></i></button>
                            <button type="button" class="btn-icon-action" title="Eliminar" data-bs-toggle="modal" data-bs-target="#eliminarMaqModal{{ $maqId }}"><i class="bi bi-trash3"></i></button>
                        </td>
                    </tr>
                    <div class="modal fade" id="verMaqModal{{ $maqId }}" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog modal-lg modal-dialog-centered"><div class="modal-content">
                            <div class="modal-header"><h5 class="modal-title">{{ $maquinaria->descripcion ?? 'Maquinaria' }}</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button></div>
                            <div class="modal-body"><div class="detail-grid">
                                <div class="detail-box"><h6>Información</h6><p><strong>Clave:</strong> {{ $maquinaria->clave ?? '—' }}</p><p><strong>Descripción:</strong> {{ $maquinaria->descripcion ?? '—' }}</p></div>
                                <div class="detail-box"><h6>Costos y Unidad</h6><p><strong>Costo horario:</strong> ${{ number_format((float) ($maquinaria->costo_horario ?? 0), 2) }}</p><p><strong>Unidad:</strong> {{ optional($maquinaria->unidadMedida)->nombre ?? 'N/D' }}</p></div>
                            </div></div>
                            <div class="modal-footer"><button type="button" class="btn-modal-light" data-bs-dismiss="modal">Cerrar</button><a href="{{ route('maquinaria_equipo.edit', $maqId) }}" class="btn-modal-dark">Editar</a></div>
                        </div></div>
                    </div>
                    <div class="modal fade" id="eliminarMaqModal{{ $maqId }}" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered"><div class="modal-content">
                            <div class="modal-header"><h5 class="modal-title">Eliminar equipo</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button></div>
                            <div class="modal-body">¿Deseas eliminar <strong>{{ $maquinaria->descripcion ?? 'este equipo' }}</strong>?<br><br>Esta acción no se puede deshacer.</div>
                            <div class="modal-footer"><button type="button" class="btn-modal-light" data-bs-dismiss="modal">Cancelar</button><form action="{{ route('maquinaria_equipo.destroy', $maqId) }}" method="POST">@csrf @method('DELETE')<button type="submit" class="btn-modal-danger">Eliminar</button></form></div>
                        </div></div>
                    </div>
                @empty
                    <tr class="project-row"><td colspan="4" class="empty-state">No hay maquinaria o equipo registrado.</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
