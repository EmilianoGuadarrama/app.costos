...
@section('content')
    <style>
        ...
        .table-custom{width:100%;min-width:900px;margin:0;border-collapse:separate;border-spacing:0}
    ...
    <div class="page-header">
        <div>
            <h2 class="page-title">Mano de Obra</h2>
            <p class="page-subtitle">Listado general de mano de obra registrada.</p>
        </div>

        <div class="toolbar">
            <button class="btn btn-soft" type="button"><i class="bi bi-funnel me-1"></i> Filtrar</button>
            <a href="{{ Route::has('mano_obra.create') ? route('mano_obra.create') : '#' }}" class="btn btn-dark-custom">
    ...
    <div class="panel-card">
        <div class="table-wrap">
            <table class="table-custom">
                ...
                <tbody>
                @forelse(($manoDeObras ?? []) as $mano)
                    <tr>
                        <td>{{ $mano->clave ?? '' }}</td>
                        <td>{{ $mano->categoria ?? '' }}</td>
                        <td>{{ $mano->unidad ?? '' }}</td>
                        <td>{{ $mano->salario ?? '' }}</td>
                        <td>
                            <div class="action-group">
                                <a class="action-btn view" href="{{ Route::has('mano_obra.show') ? route('mano_obra.show', $mano->id ?? 1) : '#' }}" title="Ver"><i class="bi bi-eye"></i></a>
                                <a class="action-btn edit" href="{{ Route::has('mano_obra.edit') ? route('mano_obra.edit', $mano->id ?? 1) : '#' }}" title="Editar"><i class="bi bi-pencil"></i></a>
                                <form action="{{ Route::has('mano_obra.destroy') ? route('mano_obra.destroy', $mano->id ?? 1) : '#' }}" method="POST" onsubmit="return confirm('¿Deseas eliminar este registro?');" style="display:inline-block;">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    @for($i = 1; $i <= 6; $i++)
                        <tr>
                            ...
                            <td>
                                <div class="action-group">
                                    <a class="action-btn view" href="{{ Route::has('mano_obra.show') ? route('mano_obra.show', $i) : '#' }}" title="Ver"><i class="bi bi-eye"></i></a>
                                    <a class="action-btn edit" href="{{ Route::has('mano_obra.edit') ? route('mano_obra.edit', $i) : '#' }}" title="Editar"><i class="bi bi-pencil"></i></a>
