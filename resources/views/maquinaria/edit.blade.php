...
<div class="dash-form-view">
    <a href="{{ route('maquinaria.index') }}" class="btn-back"><i class="bi bi-arrow-left"></i> Volver</a>
    <div class="form-panel">
        <div class="header-section"><h1>Editar Maquinaria</h1></div>
        <form action="{{ route('maquinaria.update', $maquinaria->id) }}" method="POST">
            @csrf @method('PUT')
            <div class="form-group">
                <label for="nombre">Nombre *</label>
                <input type="text" id="nombre" name="nombre" class="form-control" value="{{ old('nombre', $maquinaria->nombre) }}" required maxlength="255">
                @error('nombre') <span class="text-danger">{{ $message }}</span> @enderror
            </div>
            <div class="form-group">
                <label for="descripcion">Descripción</label>
                <textarea id="descripcion" name="descripcion" class="form-control" rows="2" maxlength="255">{{ old('descripcion', $maquinaria->descripcion) }}</textarea>
            </div>
            <div class="form-group">
                ...
                <select id="id_unidad_medida" name="id_unidad_medida" class="form-select" required>
                    ...
                    @foreach($unidades as $u)
                        <option value="{{ $u->id }}" {{ old('id_unidad_medida', $maquinaria->id_unidad_medida) == $u->id ? 'selected' : '' }}>
            ...
            <div class="form-group">
                ...
                <input type="number" step="0.01" min="0" id="precio_x_unidad" name="precio_x_unidad" class="form-control" value="{{ old('precio_x_unidad', $maquinaria->precio_x_unidad) }}" required>
                @error('precio_x_unidad') <span class="text-danger">{{ $message }}</span> @enderror
            </div>
            <button type="submit" class="btn-submit"><i class="bi bi-save me-1"></i> Actualizar</button>
        </form>
    </div>
</div>
@endsection
