...
<div class="dash-form-view">
    ...
    <div class="form-panel">
        <div class="header-section">
            ...
            <a href="{{ route('ingresos.edit', $ingreso) }}" class="btn btn-sm btn-outline-dark"><i class="bi bi-pencil"></i> Editar</a>
        </div>
        <div class="detail-row"><div class="detail-label">Concepto</div><div class="detail-value">{{ $ingreso->concepto }}</div></div>
        <div class="detail-row"><div class="detail-label">Proyecto</div><div class="detail-value">{{ $ingreso->proyecto->nombre ?? 'N/A' }}</div></div>
        <div class="detail-row"><div class="detail-label">Cliente</div><div class="detail-value">{{ $ingreso->cliente->nombre ?? 'N/A' }}</div></div>
        <div class="detail-row"><div class="detail-label">Monto</div><div class="detail-value text-success">${{ number_format($ingreso->monto, 2) }}</div></div>
        <div class="detail-row"><div class="detail-label">Fecha</div><div class="detail-value">{{ \Carbon\Carbon::parse($ingreso->fecha)->format('d/m/Y') }}</div></div>
        <div class="detail-row"><div class="detail-label">Comprobante Path</div><div class="detail-value">{{ $ingreso->comprobante ?? '—' }}</div></div>
        <div class="detail-row"><div class="detail-label">Creado</div><div class="detail-value">{{ $ingreso->created_at?->format('d/m/Y H:i') ?? '—' }}</div></div>
    </div>
</div>
@endsection
