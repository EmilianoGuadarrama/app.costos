...
<div class="dash-index-view">
    <div class="index-panel">
        ...
        <div class="header-section">
            <div><h1>Proveedores</h1><p>Catálogo de proveedores registrados en el sistema.</p></div>
            <div class="header-actions"><a href="{{ route('proveedores.create') }}" class="btn-add-new"><i class="bi bi-plus-circle me-1"></i> Nuevo Proveedor</a></div>
        </div>
        <div class="table-responsive">
            <table class="project-table">
                <thead><tr><th>Nombre</th><th>Contacto</th><th>Tipo</th><th>Email / Tel</th><th style="text-align:right;">Acciones</th></tr></thead>
                <tbody>
                @forelse($proveedores as $proveedore)
                    @php $provId = $proveedore->id; @endphp
                    <tr class="project-row">
                        <td><strong>{{ $proveedore->nombre }}</strong></td>
                        <td>{{ $proveedore->contacto ?? '—' }}</td>
                        <td>@if($proveedore->tipo)<span class="badge-soft">{{ $proveedore->tipo }}</span>@else — @endif</td>
                        <td>{{ $proveedore->correo ?? '—' }}<br><small class="text-muted">{{ $proveedore->telefono ?? '' }}</small></td>
                        <td class="action-cell">
                            <button type="button" class="btn-icon-action" title="Ver" data-bs-toggle="modal" data-bs-target="#verProvModal{{ $provId }}"><i class="bi bi-eye"></i></button>
                            <button type="button" class="btn-icon-action" title="Eliminar" data-bs-toggle="modal" data-bs-target="#eliminarProvModal{{ $provId }}"><i class="bi bi-trash3"></i></button>
                        </td>
                    </tr>
                    <div class="modal fade" id="verProvModal{{ $provId }}" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog modal-lg modal-dialog-centered"><div class="modal-content">
                            <div class="modal-header"><h5 class="modal-title">{{ $proveedore->nombre }}</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button></div>
                            <div class="modal-body"><div class="detail-grid">
                                <div class="detail-box"><h6>Datos del proveedor</h6><p><strong>Nombre:</strong> {{ $proveedore->nombre }}</p><p><strong>Contacto:</strong> {{ $proveedore->contacto ?? '—' }}</p><p><strong>Tipo:</strong> {{ $proveedore->tipo ?? '—' }}</p></div>
                                <div class="detail-box"><h6>Comunicación</h6><p><strong>Correo:</strong> {{ $proveedore->correo ?? '—' }}</p><p><strong>Teléfono:</strong> {{ $proveedore->telefono ?? '—' }}</p><p><strong>Dirección:</strong> {{ $proveedore->direccion ?? '—' }}</p></div>
                            </div></div>
                            <div class="modal-footer"><button type="button" class="btn-modal-light" data-bs-dismiss="modal">Cerrar</button><a href="{{ route('proveedores.edit', $provId) }}" class="btn-modal-dark">Editar</a></div>
                        </div></div>
                    </div>
                    <div class="modal fade" id="eliminarProvModal{{ $provId }}" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered"><div class="modal-content">
                            <div class="modal-header"><h5 class="modal-title">Eliminar proveedor</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button></div>
                            <div class="modal-body">¿Deseas eliminar el proveedor <strong>{{ $proveedore->nombre }}</strong>?<br><br>Esta acción no se puede deshacer.</div>
                            <div class="modal-footer"><button type="button" class="btn-modal-light" data-bs-dismiss="modal">Cancelar</button><form action="{{ route('proveedores.destroy', $provId) }}" method="POST">@csrf @method('DELETE')<button type="submit" class="btn-modal-danger">Eliminar</button></form></div>
                        </div></div>
                    </div>
                @empty
                    <tr class="project-row"><td colspan="5" class="empty-state">No hay proveedores registrados por el momento.</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
