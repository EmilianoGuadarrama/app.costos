    ...
    <div class="index-panel">
        ...
        <div class="table-responsive">
            <table class="project-table">
                <thead><tr><th>Detalle del presupuesto</th><th>Cantidades</th><th>Importe</th><th style="text-align:right;">Acciones</th></tr></thead>
                <tbody>
                @forelse(($presupuestos ?? []) as $presupuesto)
                    @php
                        ...
                        $cantidadTotal = $tieneDetalles ? $detalles->sum(fn($d) => (float) ($d->cantidad ?? 0)) : null;
                        $importeCalculado = $tieneDetalles ? $detalles->sum(fn($d) => (float) ($d->cantidad ?? 0) * (float) ($d->pu_unitario_snapshot ?? 0)) : null;
                        $importeFinal = $tieneDetalles ? $importeCalculado : ((float) ($presupuesto->total ?? 0) > 0 ? (float) $presupuesto->total : null);
                        $estado = strtolower(trim($presupuesto->estado ?? 'borrador'));
                        $estadoClase = match($estado) { 'aprobado' => 'badge-aprobado', 'rechazado' => 'badge-rechazado', default => 'badge-borrador' };
                    @endphp
                    <tr class="project-row">
                        <td>
                            <div class="title-main">{{ $presupuesto->nombre ?? 'Presupuesto sin nombre' }}<span class="badge-dark-mini">PRE</span></div>
                    ...
                    <div class="modal fade" id="verPresModal{{ $presId }}" tabindex="-1" aria-hidden="true">
                    ...
                    <div class="modal fade" id="eliminarPresModal{{ $presId }}" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered"><div class="modal-content">
                            <div class="modal-header"><h5 class="modal-title">Eliminar presupuesto</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button></div>
                            <div class="modal-body">¿Deseas eliminar el presupuesto <strong>{{ $presupuesto->nombre ?? '' }}</strong>?<br><br>Esta acción no se puede deshacer.</div>
                            <div class="modal-footer"><button type="button" class="btn-modal-light" data-bs-dismiss="modal">Cancelar</button><form action="{{ route('presupuestos.destroy', $presId) }}" method="POST">@csrf @method('DELETE')<button type="submit" class="btn-modal-danger">Eliminar</button></form></div>
                        </div></div>
                    </div>
                @empty
                    <tr class="project-row"><td colspan="4" class="empty-state">No hay presupuestos registrados por el momento.</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
