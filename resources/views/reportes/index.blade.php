...
<style>
    ...
    .btn-filter{ background:#fff; color:#111; border:1px solid #d9d9d9; padding:10px 16px; border-radius:6px; font-size:.8rem; letter-spacing:1px; text-transform:uppercase; font-family:Arial,sans-serif; cursor:pointer; }
    ...
    @media (max-width: 992px){ .index-panel{ padding:24px; } .project-table{ min-width:1080px; } .detail-grid{ grid-template-columns:1fr; } }
</style>
...
<div class="dash-index-view">
    <div class="index-panel">
        ...
        <div class="header-section">
            <div><h1>Reportes</h1><p>Listado de reportes de presupuestos, conceptos y totales generados.</p></div>
            <div class="header-actions">
                <button class="btn-filter" type="button"><i class="bi bi-funnel me-1"></i> Filtrar</button>
                <a href="{{ Route::has('reportes.create') ? route('reportes.create') : '#' }}" class="btn-add-new"><i class="bi bi-plus-circle"></i> Nuevo Reporte</a>
            </div>
        </div>
        <div class="table-responsive">
            <table class="project-table">
                <thead><tr><th style="width:35%;">Detalle del Reporte</th><th>Totales</th><th>Fecha</th><th style="text-align:right;">Acciones</th></tr></thead>
                <tbody>
                @forelse(($reportes ?? []) as $reporte)
                    @php $reporteId = $reporte->id ?? $reporte->id_reporte ?? $reporte->id_presupuesto ?? 1; @endphp
                    <tr class="project-row">
                        <td>
                            <div class="title-main">{{ $reporte->nombre ?? 'Reporte sin nombre' }}<span class="badge-dark-mini">#{{ $reporteId }}</span></div>
                            <div class="desc-text">Presupuesto relacionado: {{ $reporte->id_presupuesto ?? 'No definido' }}</div>
                        </td>
                        <td><div class="info-stack"><div><strong>Total conceptos:</strong> {{ $reporte->total_conceptos ?? '0' }}</div><div><strong>Importe total:</strong> {{ $reporte->importe_total ?? '0.00' }}</div></div></td>
                        <td><span class="badge-soft">{{ $reporte->fecha ?? 'Sin fecha' }}</span></td>
                        <td class="action-cell">
                            <button type="button" class="btn-icon-action" title="Ver" data-bs-toggle="modal" data-bs-target="#verRepModal{{ $reporteId }}"><i class="bi bi-eye"></i></button>
                            <button type="button" class="btn-icon-action" title="Eliminar" data-bs-toggle="modal" data-bs-target="#eliminarRepModal{{ $reporteId }}"><i class="bi bi-trash3"></i></button>
                        </td>
                    </tr>
                    <div class="modal fade" id="verRepModal{{ $reporteId }}" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog modal-lg modal-dialog-centered"><div class="modal-content">
                            <div class="modal-header"><h5 class="modal-title">{{ $reporte->nombre ?? 'Reporte' }}</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button></div>
                            <div class="modal-body"><div class="detail-grid">
                                <div class="detail-box"><h6>Información</h6><p><strong>Nombre:</strong> {{ $reporte->nombre ?? '—' }}</p><p><strong>Presupuesto:</strong> {{ $reporte->id_presupuesto ?? '—' }}</p><p><strong>Fecha:</strong> {{ $reporte->fecha ?? '—' }}</p></div>
                                <div class="detail-box"><h6>Totales</h6><p><strong>Total conceptos:</strong> {{ $reporte->total_conceptos ?? '0' }}</p><p><strong>Importe total:</strong> {{ $reporte->importe_total ?? '0.00' }}</p></div>
                            </div></div>
                            <div class="modal-footer"><button type="button" class="btn-modal-light" data-bs-dismiss="modal">Cerrar</button><a href="{{ Route::has('reportes.edit') ? route('reportes.edit', $reporteId) : '#' }}" class="btn-modal-dark">Editar</a></div>
                        </div></div>
                    </div>
                    <div class="modal fade" id="eliminarRepModal{{ $reporteId }}" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered"><div class="modal-content">
                            <div class="modal-header"><h5 class="modal-title">Eliminar reporte</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button></div>
                            <div class="modal-body">¿Deseas eliminar el reporte <strong>{{ $reporte->nombre ?? '' }}</strong>?<br><br>Esta acción no se puede deshacer.</div>
                            <div class="modal-footer"><button type="button" class="btn-modal-light" data-bs-dismiss="modal">Cancelar</button>
                                @if(Route::has('reportes.destroy'))
                                <form action="{{ route('reportes.destroy', $reporteId) }}" method="POST">@csrf @method('DELETE')<button type="submit" class="btn-modal-danger">Eliminar</button></form>
                                @else
                                <button type="button" class="btn-modal-danger" disabled>Eliminar</button>
                                @endif
                            </div>
                        </div></div>
                    </div>
                @empty
                    <tr class="project-row"><td colspan="4" class="empty-state">No hay reportes generados todavía.</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
