@extends('layout')
@section('title','Editar Responsable Técnico')
@section('content')
...
<div class="dash-form-view">
    ...
    <div class="form-panel">
        <div class="header-section"><h1>Editar Responsable Técnico</h1></div>
        <form action="{{ route('responsables_tecnicos.update', $responsable) }}" method="POST">
            @csrf @method('PUT')
            <div class="form-group">
                ...
                <select id="empresa_id" name="empresa_id" class="form-select" required>
                    ...
                    @foreach($empresas as $e)
                        <option value="{{ $e->id }}" {{ old('empresa_id', $responsable->empresa_id) == $e->id ? 'selected' : '' }}>{{ $e->nombre }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input type="text" id="nombre" name="nombre" class="form-control" value="{{ old('nombre', $responsable->nombre) }}" required maxlength="150">
                @error('nombre') <span class="text-danger">{{ $message }}</span> @enderror
            </div>
            <div class="form-group">
                <label for="cargo">Cargo *</label>
                <input type="text" id="cargo" name="cargo" class="form-control" value="{{ old('cargo', $responsable->cargo) }}" required maxlength="120">
                @error('cargo') <span class="text-danger">{{ $message }}</span> @enderror
            </div>
            <div class="form-group">
                <label for="firma_path">Firma (Ruta/URL)</label>
                <input type="text" id="firma_path" name="firma_path" class="form-control" value="{{ old('firma_path', $responsable->firma_path) }}" maxlength="255">
                @error('firma_path') <span class="text-danger">{{ $message }}</span> @enderror
            </div>
            <button type="submit" class="btn-submit">Actualizar Responsable</button>
        </form>
    </div>
</div>
@endsection
