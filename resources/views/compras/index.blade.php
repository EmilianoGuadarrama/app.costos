...
<style>
    ...
    .badge-soft{ display:inline-flex; align-items:center; padding:4px 10px; border-radius:12px; background:#eee; color:#333; font-family:Arial,sans-serif; font-size:.75rem; font-weight:700; text-transform:uppercase;}
...
<div class="dash-index-view">
    <div class="index-panel">
        ...
        <div class="header-section">
            <div><h1>Compras</h1><p>Registro y seguimiento de compras por proyecto.</p></div>
            <div class="header-actions"><a href="{{ route('compras.create') }}" class="btn-add-new"><i class="bi bi-plus-circle me-1"></i> Nueva Compra</a></div>
        </div>
        <div class="table-responsive">
            <table class="project-table">
                <thead><tr><th>Proyecto / Área</th><th>Proveedor</th><th>Factura / Fecha</th><th>Estado</th><th style="text-align:right;">Acciones</th></tr></thead>
                <tbody>
                @forelse($compras as $compra)
                    @php $compraId = $compra->id; @endphp
                    <tr class="project-row">
                        <td><strong>{{ $compra->proyecto->nombre ?? 'N/A' }}</strong><br><small class="text-muted">{{ $compra->area->nombre ?? 'N/A' }}</small></td>
                        <td>{{ $compra->proveedor->nombre ?? 'N/A' }}</td>
                        <td>{{ $compra->factura ?? 'S/F' }}<br><small class="text-muted">{{ \Carbon\Carbon::parse($compra->fecha_compra)->format('d/m/Y') }}</small></td>
                        <td><span class="badge-soft">{{ $compra->estado }}</span></td>
                        <td class="action-cell">
                            <button type="button" class="btn-icon-action" title="Ver" data-bs-toggle="modal" data-bs-target="#verCompraModal{{ $compraId }}"><i class="bi bi-eye"></i></button>
                            <button type="button" class="btn-icon-action" title="Eliminar" data-bs-toggle="modal" data-bs-target="#eliminarCompraModal{{ $compraId }}"><i class="bi bi-trash3"></i></button>
                        </td>
                    </tr>
                    <div class="modal fade" id="verCompraModal{{ $compraId }}" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog modal-lg modal-dialog-centered"><div class="modal-content">
                            <div class="modal-header"><h5 class="modal-title">Detalle de compra</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button></div>
                            <div class="modal-body"><div class="detail-grid">
                                <div class="detail-box"><h6>Proyecto y Área</h6><p><strong>Proyecto:</strong> {{ $compra->proyecto->nombre ?? 'N/A' }}</p><p><strong>Área:</strong> {{ $compra->area->nombre ?? 'N/A' }}</p></div>
                                <div class="detail-box"><h6>Proveedor y Factura</h6><p><strong>Proveedor:</strong> {{ $compra->proveedor->nombre ?? 'N/A' }}</p><p><strong>Factura:</strong> {{ $compra->factura ?? 'S/F' }}</p><p><strong>Fecha:</strong> {{ \Carbon\Carbon::parse($compra->fecha_compra)->format('d/m/Y') }}</p><p><strong>Estado:</strong> {{ $compra->estado }}</p></div>
                            </div></div>
                            <div class="modal-footer"><button type="button" class="btn-modal-light" data-bs-dismiss="modal">Cerrar</button><a href="{{ route('compras.edit', $compraId) }}" class="btn-modal-dark">Editar</a></div>
                        </div></div>
                    </div>
                    <div class="modal fade" id="eliminarCompraModal{{ $compraId }}" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered"><div class="modal-content">
                            <div class="modal-header"><h5 class="modal-title">Eliminar compra</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button></div>
                            <div class="modal-body">¿Deseas eliminar esta compra?<br><br>Esta acción no se puede deshacer.</div>
                            <div class="modal-footer"><button type="button" class="btn-modal-light" data-bs-dismiss="modal">Cancelar</button><form action="{{ route('compras.destroy', $compraId) }}" method="POST">@csrf @method('DELETE')<button type="submit" class="btn-modal-danger">Eliminar</button></form></div>
                        </div></div>
                    </div>
                @empty
                    <tr class="project-row"><td colspan="5" class="empty-state">No hay compras registradas por el momento.</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
