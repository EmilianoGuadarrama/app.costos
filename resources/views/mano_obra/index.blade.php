        ...
        <div class="index-panel">
            <div class="header-section">
                ...
                <div class="header-actions">
                    ...
                    <a href="{{ Route::has('mano_obra.create') ? route('mano_obra.create') : '#' }}" class="btn-add-new">
            ...
            <div class="table-responsive">
                <table class="project-table">
                    <thead>
                    <tr>
                        <th style="width:35%;">Detalle de Mano de Obra</th>
                        <th>Pago</th>
                        <th>Unidad</th>
                    ...
                    <tbody>
                    @forelse(($manoDeObras ?? $mano_obra ?? []) as $mano)
                        @php $manoId = $mano->id ?? $mano->id_mano_obra ?? 1; @endphp
                        <tr class="project-row">
                            <td>
                                <div class="title-main">
                                    {{ $mano->categoria ?? 'Categoría sin nombre' }}
                                    <span class="badge-dark-mini">{{ $mano->clave ?? 'MO' }}</span>
                                </div>
                                <div class="desc-text">Registro de categoría laboral para análisis de costos.</div>
                            </td>
                            <td>
                                <div class="info-stack">
                                    <div><strong>Salario:</strong> {{ $mano->salario ?? $mano->salario_unitario ?? '0.00' }}</div>
                                </div>
                            </td>
                            <td>
                                <span class="badge-soft">{{ $mano->unidad ?? data_get($mano, 'unidadMedida.abreviatura') ?? 'N/D' }}</span>
                            </td>
                            <td class="action-cell">
                                <a href="{{ Route::has('mano_obra.show') ? route('mano_obra.show', $manoId) : '#' }}" class="btn-icon-action" title="Ver">
                                    <i class="bi bi-eye"></i>
                                </a>
                                <a href="{{ Route::has('mano_obra.edit') ? route('mano_obra.edit', $manoId) : '#' }}" class="btn-icon-action" title="Editar">
                                    <i class="bi bi-pencil-square"></i>
                                </a>
                                @if(Route::has('mano_obra.destroy'))
                                    <form action="{{ route('mano_obra.destroy', $manoId) }}" method="POST" style="display:inline;" onsubmit="return confirm('¿Eliminar este registro de mano de obra?');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr class="project-row">
                            <td colspan="4" class="empty-state">No hay registros de mano de obra disponibles.</td>
                        </tr>
                    @endforelse
                    </tbody>
                </table>
