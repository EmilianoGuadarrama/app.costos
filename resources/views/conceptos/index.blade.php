        ...
        <div class="index-panel">
            ...
            <div class="header-section">
                ...
                <div class="header-actions">
                    ...
                    <a href="{{ route('conceptos.create') }}" class="btn-add-new"><i class="bi bi-plus-circle me-1"></i> Nuevo Concepto</a>
                </div>
            </div>

            <div class="table-responsive">
                <table class="project-table">
                    <thead><tr><th style="width:35%;">Detalle del Concepto</th><th>Cantidades y Costos</th><th>Clasificación</th><th style="text-align:right;">Acciones</th></tr></thead>
                    <tbody>
                    @forelse($conceptos as $concepto)
                        ...
                        <div class="modal fade" id="verConceptoModal{{ $conceptoId }}" tabindex="-1" aria-hidden="true">
                        ...
                        <div class="modal fade" id="eliminarConceptoModal{{ $conceptoId }}" tabindex="-1" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered"><div class="modal-content">
                                <div class="modal-header"><h5 class="modal-title">Eliminar concepto</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button></div>
                                <div class="modal-body">¿Deseas eliminar el concepto <strong>{{ $concepto->descripcion }}</strong>?<br><br>Esta acción no se puede deshacer.</div>
                                <div class="modal-footer"><button type="button" class="btn-modal-light" data-bs-dismiss="modal">Cancelar</button><form action="{{ route('conceptos.destroy', $conceptoId) }}" method="POST">@csrf @method('DELETE')<button type="submit" class="btn-modal-danger">Eliminar</button></form></div>
                            </div></div>
                        </div>
                    @empty
                        <tr class="project-row"><td colspan="4" class="empty-state">No hay conceptos registrados por el momento.</td></tr>
                    @endforelse
                    </tbody>
                </table>
            </div>
