@extends('layout')
@section('title','Detalle Estado de Proyecto')
@section('content')
...
<div class="dash-form-view">
    <a href="{{ route('estados_proyecto.index') }}" class="btn-back"><i class="bi bi-arrow-left"></i> Volver</a>
    <div class="form-panel">
        <div class="header-section">
            <h1>{{ $estado->nombre }}</h1>
            <a href="{{ route('estados_proyecto.edit', $estado) }}" class="btn btn-sm btn-outline-dark"><i class="bi bi-pencil"></i> Editar</a>
        </div>
        <div class="detail-row"><div class="detail-label">ID</div><div class="detail-value">{{ $estado->id }}</div></div>
        <div class="detail-row"><div class="detail-label">Nombre</div><div class="detail-value">{{ $estado->nombre }}</div></div>
        <div class="detail-row"><div class="detail-label">Creado</div><div class="detail-value">{{ $estado->created_at?->format('d/m/Y H:i') ?? '—' }}</div></div>
    </div>
</div>
@endsection
