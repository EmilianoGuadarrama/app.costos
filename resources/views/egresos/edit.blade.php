...
<div class="dash-form-view">
    ...
    <div class="form-panel">
        <div class="header-section"><h1>Editar Egreso</h1></div>
        <form action="{{ route('egresos.update', $egreso) }}" method="POST">
            ...
            <div class="form-group">
                ...
                <select id="proyecto_id" name="proyecto_id" class="form-select" required>
                    ...
                    @foreach($proyectos as $p)
                        <option value="{{ $p->id }}" {{ old('proyecto_id', $egreso->proyecto_id) == $p->id ? 'selected' : '' }}>{{ $p->nombre }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select id="categoria_id" name="categoria_id" class="form-select" required>
                    ...
                    @foreach($categorias as $c)
                        <option value="{{ $c->id }}" {{ old('categoria_id', $egreso->categoria_id) == $c->id ? 'selected' : '' }}>{{ $c->nombre }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input type="text" id="concepto" name="concepto" class="form-control" value="{{ old('concepto', $egreso->concepto) }}" required maxlength="255">
                @error('concepto') <span class="text-danger">{{ $message }}</span> @enderror
            </div>
            <div class="form-group">
                <label for="monto">Monto *</label>
                <input type="number" step="0.01" id="monto" name="monto" class="form-control" value="{{ old('monto', $egreso->monto) }}" required>
                @error('monto') <span class="text-danger">{{ $message }}</span> @enderror
            </div>
            <div class="form-group">
                <label for="fecha">Fecha *</label>
                <input type="date" id="fecha" name="fecha" class="form-control" value="{{ old('fecha', \Carbon\Carbon::parse($egreso->fecha)->format('Y-m-d')) }}" required>
                @error('fecha') <span class="text-danger">{{ $message }}</span> @enderror
            </div>
            <div class="form-group">
                <label for="comprobante">Comprobante (Ruta/URL)</label>
                <input type="text" id="comprobante" name="comprobante" class="form-control" value="{{ old('comprobante', $egreso->comprobante) }}" maxlength="255">
                @error('comprobante') <span class="text-danger">{{ $message }}</span> @enderror
            </div>
            <button type="submit" class="btn-submit">Actualizar Egreso</button>
        </form>
    </div>
</div>
@endsection
