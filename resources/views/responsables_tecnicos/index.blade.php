@extends('layout')
@section('title','Responsables Técnicos')
@section('content')
...
<div class="dash-index-view">
    <div class="index-panel">
        ...
        <div class="header-section">
            <div><h1>Responsables Técnicos</h1><p>Catálogo de responsables técnicos asignados a proyectos.</p></div>
            <div class="header-actions"><a href="{{ route('responsables_tecnicos.create') }}" class="btn-add-new"><i class="bi bi-plus-circle me-1"></i> Nuevo Responsable</a></div>
        </div>
        <div class="table-responsive">
            <table class="project-table">
                <thead><tr><th>Nombre</th><th>Cargo</th><th>Empresa</th><th style="text-align:right;">Acciones</th></tr></thead>
                <tbody>
                @forelse($responsables as $responsable)
                    @php $respId = $responsable->id; @endphp
                    <tr class="project-row">
                        <td><strong>{{ $responsable->nombre }}</strong></td>
                        <td>{{ $responsable->cargo }}</td>
                        <td>{{ $responsable->empresa->nombre ?? 'N/A' }}</td>
                        <td class="action-cell">
                            <button type="button" class="btn-icon-action" title="Ver" data-bs-toggle="modal" data-bs-target="#verRespModal{{ $respId }}"><i class="bi bi-eye"></i></button>
                            <button type="button" class="btn-icon-action" title="Eliminar" data-bs-toggle="modal" data-bs-target="#eliminarRespModal{{ $respId }}"><i class="bi bi-trash3"></i></button>
                        </td>
                    </tr>
                    <div class="modal fade" id="verRespModal{{ $respId }}" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog modal-lg modal-dialog-centered"><div class="modal-content">
                            <div class="modal-header"><h5 class="modal-title">{{ $responsable->nombre }}</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button></div>
                            <div class="modal-body"><div class="detail-grid">
                                <div class="detail-box"><h6>Datos personales</h6><p><strong>Nombre:</strong> {{ $responsable->nombre }}</p><p><strong>Cargo:</strong> {{ $responsable->cargo }}</p></div>
                                <div class="detail-box"><h6>Empresa</h6><p><strong>Empresa:</strong> {{ $responsable->empresa->nombre ?? 'N/A' }}</p><p><strong>Firma:</strong> {{ $responsable->firma_path ?? 'Sin firma registrada' }}</p></div>
                            </div></div>
                            <div class="modal-footer"><button type="button" class="btn-modal-light" data-bs-dismiss="modal">Cerrar</button><a href="{{ route('responsables_tecnicos.edit', $respId) }}" class="btn-modal-dark">Editar</a></div>
                        </div></div>
                    </div>
                    <div class="modal fade" id="eliminarRespModal{{ $respId }}" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered"><div class="modal-content">
                            <div class="modal-header"><h5 class="modal-title">Eliminar responsable</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button></div>
                            <div class="modal-body">¿Deseas eliminar al responsable <strong>{{ $responsable->nombre }}</strong>?<br><br>Esta acción no se puede deshacer.</div>
                            <div class="modal-footer"><button type="button" class="btn-modal-light" data-bs-dismiss="modal">Cancelar</button><form action="{{ route('responsables_tecnicos.destroy', $respId) }}" method="POST">@csrf @method('DELETE')<button type="submit" class="btn-modal-danger">Eliminar</button></form></div>
                        </div></div>
                    </div>
                @empty
                    <tr class="project-row"><td colspan="4" class="empty-state">No hay responsables técnicos registrados por el momento.</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
