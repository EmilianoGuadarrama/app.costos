...
@section('content')
    <style>
        ...
        .table-custom{width:100%;min-width:950px;margin:0;border-collapse:separate;border-spacing:0}
    ...
    <div class="page-header">
        <div>
            <h2 class="page-title">Maquinaria y Equipo</h2>
            <p class="page-subtitle">Listado general de maquinaria y equipo registrado.</p>
        </div>

        <div class="toolbar">
            <button class="btn btn-soft" type="button"><i class="bi bi-funnel me-1"></i> Filtrar</button>
            <a href="{{ Route::has('maquinaria_equipo.create') ? route('maquinaria_equipo.create') : '#' }}" class="btn btn-dark-custom">
            ...
            <table class="table-custom">
                ...
                @forelse(($maquinarias ?? []) as $maquinaria)
                    <tr>
                        ...
                        <td>{{ $maquinaria->costo_por_hora ?? '' }}</td>
                        <td>
                            <div class="action-group">
                                <a class="action-btn view" href="{{ Route::has('maquinaria_equipo.show') ? route('maquinaria_equipo.show', $maquinaria->id ?? 1) : '#' }}" title="Ver"><i class="bi bi-eye"></i></a>
                                <a class="action-btn edit" href="{{ Route::has('maquinaria_equipo.edit') ? route('maquinaria_equipo.edit', $maquinaria->id ?? 1) : '#' }}" title="Editar"><i class="bi bi-pencil"></i></a>
                                <form action="{{ Route::has('maquinaria_equipo.destroy') ? route('maquinaria_equipo.destroy', $maquinaria->id ?? 1) : '#' }}" method="POST" onsubmit="return confirm('¿Deseas eliminar este registro?');" style="display:inline-block;">
                                    @csrf
                                    @method('DELETE')
                                    <button class="action-btn delete" type="submit" title="Eliminar"><i class="bi bi-trash"></i></button>
                                </form>
                            </div>
                        </td>
                    </tr>
                @empty
                    @for($i = 1; $i <= 6; $i++)
                        <tr>
                            <td class="empty-text">—</td><td class="empty-text">—</td><td class="empty-text">—</td><td class="empty-text">—</td>
                            <td>
                                <div class="action-group">
                                    <a class="action-btn view" href="{{ Route::has('maquinaria_equipo.show') ? route('maquinaria_equipo.show', $i) : '#' }}" title="Ver"><i class="bi bi-eye"></i></a>
                                    <a class="action-btn edit" href="{{ Route::has('maquinaria_equipo.edit') ? route('maquinaria_equipo.edit', $i) : '#' }}" title="Editar"><i class="bi bi-pencil"></i></a>
                                    <button class="action-btn delete" type="button" title="Eliminar"><i class="bi bi-trash"></i></button>
                                </div>
                            </td>
                        </tr>
                    @endfor
