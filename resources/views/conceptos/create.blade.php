...
.um-panel label{font-size:.72rem;font-weight:700;color:#92400e;display:block;margin-bottom:3px;}
...
<div class="cc-wrap">
    ...
    <div class="cc-header mt-3">
        <div>
            <h1 class="cc-title"><i class="bi bi-card-list me-2"></i>Nuevo Concepto</h1>
            <p class="cc-sub">Registra el concepto y su composición de materiales, maquinaria y mano de obra.</p>
        </div>
    </div>

    <form action="{{ route('conceptos.store') }}" method="POST" id="formConcepto">
        @csrf
        ...
        <div class="card-sec">
            ...
            <div class="fg-1" style="margin-bottom:14px;">
                <div>
                    ...
                    <textarea name="descripcion" class="fc" rows="2" required maxlength="255"
                              placeholder="Ej. Excavación en terreno natural con retroexcavadora...">{{ old('descripcion') }}</textarea>
                </div>
            </div>
            ...
            <div class="fg-2" style="margin-bottom:14px;">
                ...
                <div>
                    ...
                    <input type="number" step="0.01" min="0" name="p_u" class="fc" value="{{ old('p_u',0) }}" required>
                </div>
            </div>
            ...
            <div class="fg-2">
                <div>
                    ...
                    <div class="ac-wrap">
                        <input type="text" id="um_texto" class="fc ac-input" placeholder="Escribir o buscar UM…" autocomplete="off"
                               value="{{ $unidades->find(old('id_unidad_medida'))?->abreviatura ?? '' }}">
                        <div class="ac-list" id="um_list"></div>
                    </div>
                    <input type="hidden" name="id_unidad_medida" id="um_id" value="{{ old('id_unidad_medida') }}">
                    <div class="um-panel" id="umNuevoPanel">
                        <label>Registrar nueva unidad de medida:</label>
                        <div class="um-row">
                            <div><label>Abreviatura</label><input type="text" id="umNuevoAbr" class="inp-s" placeholder="Ej. m²"></div>
                            <div><label>Nombre completo</label><input type="text" id="umNuevoNom" class="inp-s" placeholder="Ej. Metro cuadrado"></div>
                ...
                <div>
                    ...
                    <input type="number" step="1" min="0" name="duracion_en_dias" class="fc" value="{{ old('duracion_en_dias') }}" placeholder="0">
                </div>
            </div>
        </div>
        ...
        {{-- ── Composición ── --}}
        <div class="card-sec">
            <div class="sec-title"><i class="bi bi-diagram-3"></i>Composición del Concepto <small style="font-size:.7rem;color:#9ca3af;font-weight:400;margin-left:8px;">Qué materiales, maquinaria y mano de obra implica</small></div>
            ...
            <table class="comp-table" id="compTable">
                ...
                <tbody id="compBody">
                    <tr id="compEmpty"><td colspan="4" style="text-align:center;color:#9ca3af;padding:16px;font-size:.82rem;"><i class="bi bi-plus-circle me-1"></i>Usa los botones para agregar insumos al concepto</td></tr>
                </tbody>
            </table>
        </div>
        ...
        <div class="form-actions">
            ...
            <button type="submit" class="btn-save" id="btn-guardar-concepto"><i class="bi bi-check-lg me-1"></i>Guardar Concepto</button>
        </div>
    </form>
</div>
...
umInput.addEventListener('input', function() {
    ...
    renderUMList(filtered, q);
});

function renderUMList(items, q) {
    umList.innerHTML = '';
    items.slice(0,12).forEach(u => {
...
function addComp(tipo) {
    ...
    tr.innerHTML = `
        <td>${badge}
            ...
            <input type="hidden" name="composicion[${ri}][referencia_id]" id="comp_ref_${ri}" value="">
        </td>
        <td>
            <div class="ac-wrap">
                <input type="text" class="inp-s" id="comp_txt_${ri}" placeholder="Escribir para buscar..." autocomplete="off">
                <div class="ac-list" id="comp_list_${ri}"></div>
            </div>
        </td>
        <td><input type="number" step="0.01" min="0" name="composicion[${ri}][cantidad]" class="inp-s" value="1" style="width:80px;"></td>
