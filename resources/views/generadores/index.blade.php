...
@section('content')
    <style>
        ...
        .table-custom{width:100%;min-width:1350px;margin:0;border-collapse:separate;border-spacing:0}
    ...
    <div class="page-header">
        <div>
            <h2 class="page-title">Generadores</h2>
            <p class="page-subtitle">Listado general de generadores registrados.</p>
        </div>

        <div class="toolbar">
            <button class="btn btn-soft" type="button"><i class="bi bi-funnel me-1"></i> Filtrar</button>
            <a href="{{ Route::has('generadores.create') ? route('generadores.create') : '#' }}" class="btn btn-dark-custom">
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="action-group">
                                <a class="action-btn view" href="{{ Route::has('generadores.show') ? route('generadores.show', $generador->id ?? 1) : '#' }}" title="Ver">
                                    <i class="bi bi-eye"></i>
                                </a>
                                <a class="action-btn edit" href="{{ Route::has('generadores.edit') ? route('generadores.edit', $generador->id ?? 1) : '#' }}" title="Editar">
                                    <i class="bi bi-pencil"></i>
                                </a>
                                <form action="{{ Route::has('generadores.destroy') ? route('generadores.destroy', $generador->id ?? 1) : '#' }}" method="POST" onsubmit="return confirm('¿Deseas eliminar este generador?');" style="display:inline-block;">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    @for($i = 1; $i <= 6; $i++)
                        <tr>
                            ...
                            <td class="empty-text">—</td><td class="empty-text">—</td><td class="empty-text">—</td><td class="empty-text">—</td><td class="empty-text">—</td>
                            <td>
                                <div class="action-group">
                                    <a class="action-btn view" href="{{ Route::has('generadores.show') ? route('generadores.show', $i) : '#' }}" title="Ver"><i class="bi bi-eye"></i></a>
                                    <a class="action-btn edit" href="{{ Route::has('generadores.edit') ? route('generadores.edit', $i) : '#' }}" title="Editar"><i class="bi bi-pencil"></i></a>
