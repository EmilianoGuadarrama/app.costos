...
@section('content')
    ...
    @php
        $campos = [
            ['name' => 'clave', 'label' => 'Clave', 'value' => old('clave', $maquinaria->clave ?? 'MQ-001')],
            ['name' => 'equipo', 'label' => 'Equipo', 'value' => old('equipo', $maquinaria->equipo ?? 'Retroexcavadora')],
            ['name' => 'unidad', 'label' => 'Unidad', 'value' => old('unidad', $maquinaria->unidad ?? 'Hr')],
            ['name' => 'costo_por_hora', 'label' => 'Costo por hora', 'value' => old('costo_por_hora', $maquinaria->costo_por_hora ?? '350.00')],
        ];
    @endphp

    <div class="page-header">
        <div>
            <h2 class="page-title">Editar Maquinaria y Equipo</h2>
            <p class="page-subtitle">Modifica la información registrada.</p>
        </div>
    ...
    <div class="form-card">
        <form action="{{ Route::has('maquinaria_equipo.update') ? route('maquinaria_equipo.update', $maquinaria->id ?? 1) : '#' }}" method="POST" class="form-grid">
            @csrf
            @method('PUT')
