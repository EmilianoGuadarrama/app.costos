@extends('layout')
@section('title','Editar Caja Chica')
@section('content')
...
<div class="dash-form-view">
    ...
    <div class="form-panel">
        <div class="header-section"><h1>Editar Caja Chica</h1></div>
        <form action="{{ route('cajas_chicas.update', $cajas_chica) }}" method="POST">
            @csrf @method('PUT')
            <div class="form-group">
                ...
                <select id="proyecto_id" name="proyecto_id" class="form-select" required>
                    ...
                    @foreach($proyectos as $p)
                        <option value="{{ $p->id }}" {{ old('proyecto_id', $cajas_chica->proyecto_id) == $p->id ? 'selected' : '' }}>{{ $p->nombre }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select id="responsable_id" name="responsable_id" class="form-select" required>
                    ...
                    @foreach($responsables as $r)
                        <option value="{{ $r->id }}" {{ old('responsable_id', $cajas_chica->responsable_id) == $r->id ? 'selected' : '' }}>{{ $r->nombre }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input type="text" id="nombre" name="nombre" class="form-control" value="{{ old('nombre', $cajas_chica->nombre) }}" required maxlength="150">
                @error('nombre') <span class="text-danger">{{ $message }}</span> @enderror
            </div>
            <div class="form-group">
                <label>Monto Inicial</label>
                <input type="text" class="form-control" value="${{ number_format($cajas_chica->monto_inicial, 2) }}" readonly style="background:#f0f0f0;">
                <small class="text-muted">El monto inicial no puede ser modificado aquí.</small>
            </div>
            <button type="submit" class="btn-submit">Actualizar Caja Chica</button>
        </form>
    </div>
</div>
@endsection
