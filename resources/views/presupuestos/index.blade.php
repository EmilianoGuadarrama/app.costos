...
@section('content')
    ...
    <style>
        ...
        @media (max-width: 992px){ .index-panel{ padding:24px; } .project-table{ min-width:1150px; } }
    </style>
        ...
        <div class="index-panel">
            <div class="header-section">
                ...
                <div class="header-actions">
                    ...
                    <a href="{{ Route::has('presupuesto.create') ? route('presupuesto.create') : '#' }}" class="btn-add-new">
            ...
            <div class="table-responsive">
                <table class="project-table">
                    ...
                    @forelse(($presupuestos ?? []) as $presupuesto)
                        @php $presupuestoId = $presupuesto->id ?? $presupuesto->id_presupuesto ?? 1; @endphp
                        <tr class="project-row">
                            <td>
                                <div class="title-main">
                                    {{ $presupuesto->concepto ?? $presupuesto->nombre ?? 'Concepto sin nombre' }}
                                    <span class="badge-dark-mini">{{ $presupuesto->clave ?? 'PRE' }}</span>
                                </div>
                                <div class="desc-text">Unidad: {{ $presupuesto->unidad ?? 'N/D' }}</div>
                            </td>
                            <td>
                                <div class="info-stack">
                                    <div><strong>Cantidad:</strong> {{ $presupuesto->cantidad ?? '0' }}</div>
                                    <div><strong>Costo directo:</strong> {{ $presupuesto->costo_directo ?? '0.00' }}</div>
                                </div>
                            </td>
                            <td>
                                <span class="badge-soft">{{ $presupuesto->importe ?? '0.00' }}</span>
                            </td>
                            <td class="action-cell">
                                <a href="{{ Route::has('presupuesto.show') ? route('presupuesto.show', $presupuestoId) : '#' }}" class="btn-icon-action" title="Ver">
                                    <i class="bi bi-eye"></i>
                                </a>
                                <a href="{{ Route::has('presupuesto.edit') ? route('presupuesto.edit', $presupuestoId) : '#' }}" class="btn-icon-action" title="Editar">
                                    <i class="bi bi-pencil-square"></i>
                                </a>
                                @if(Route::has('presupuesto.destroy'))
                                    <form action="{{ route('presupuesto.destroy', $presupuestoId) }}" method="POST" style="display:inline;" onsubmit="return confirm('¿Eliminar este presupuesto?');">
                                        @csrf
                                        @method('DELETE')
