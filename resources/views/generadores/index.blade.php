...
<div class="dash-index-view">
    <div class="index-panel">
        ...
        <div class="header-section">
            <div><h1>Generadores</h1><p>Listado de generadores de obra por proyecto y concepto.</p></div>
            <div class="header-actions">
                <button class="btn-filter" type="button"><i class="bi bi-funnel me-1"></i> Filtrar</button>
                <a href="{{ route('generadores.create') }}" class="btn-add-new"><i class="bi bi-plus-circle me-1"></i> Nuevo Generador</a>
            </div>
        </div>
        <div class="table-responsive">
            <table class="project-table">
                <thead><tr><th style="width:35%;">Detalle del Generador</th><th>Cantidades</th><th>Clasificación</th><th style="text-align:right;">Acciones</th></tr></thead>
                <tbody>
                @forelse(($generadores ?? []) as $generador)
                    @php $genId = $generador->id; @endphp
                    <tr class="project-row">
                        <td>
                            <div class="title-main">{{ optional($generador->concepto)->clave ?? 'S/C' }}<span class="badge-dark-mini">GEN</span></div>
                            <div class="desc-text">{{ optional($generador->concepto)->descripcion ?? 'Sin descripción' }}</div>
                        </td>
                        <td><div class="info-stack"><div><strong>Cantidad:</strong> {{ $generador->cantidad ?? '0' }}</div><div><strong>Unidad:</strong> {{ optional(optional($generador->concepto)->unidadMedida)->abreviatura ?? 'N/D' }}</div></div></td>
                        <td><span class="badge-soft">{{ optional($generador->proyecto)->nombre ?? 'Sin proyecto' }}</span></td>
                        <td class="action-cell">
                            <button type="button" class="btn-icon-action" title="Ver" data-bs-toggle="modal" data-bs-target="#verGenModal{{ $genId }}"><i class="bi bi-eye"></i></button>
                            <button type="button" class="btn-icon-action" title="Eliminar" data-bs-toggle="modal" data-bs-target="#eliminarGenModal{{ $genId }}"><i class="bi bi-trash3"></i></button>
                        </td>
                    </tr>
                    <div class="modal fade" id="verGenModal{{ $genId }}" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog modal-lg modal-dialog-centered"><div class="modal-content">
                            <div class="modal-header"><h5 class="modal-title">Generador #{{ $genId }}</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button></div>
                            <div class="modal-body"><div class="detail-grid">
                                <div class="detail-box"><h6>Concepto</h6><p><strong>Clave:</strong> {{ optional($generador->concepto)->clave ?? 'S/C' }}</p><p><strong>Descripción:</strong> {{ optional($generador->concepto)->descripcion ?? '—' }}</p></div>
                                <div class="detail-box"><h6>Datos</h6><p><strong>Proyecto:</strong> {{ optional($generador->proyecto)->nombre ?? '—' }}</p><p><strong>Cantidad:</strong> {{ $generador->cantidad ?? '0' }}</p><p><strong>Observaciones:</strong> {{ $generador->observaciones ?? '—' }}</p></div>
                            </div></div>
                            <div class="modal-footer"><button type="button" class="btn-modal-light" data-bs-dismiss="modal">Cerrar</button><a href="{{ route('generadores.edit', $genId) }}" class="btn-modal-dark">Editar</a></div>
                        </div></div>
                    </div>
                    <div class="modal fade" id="eliminarGenModal{{ $genId }}" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered"><div class="modal-content">
                            <div class="modal-header"><h5 class="modal-title">Eliminar generador</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button></div>
                            <div class="modal-body">¿Deseas eliminar este generador?<br><br>Esta acción no se puede deshacer.</div>
                            <div class="modal-footer"><button type="button" class="btn-modal-light" data-bs-dismiss="modal">Cancelar</button><form action="{{ route('generadores.destroy', $genId) }}" method="POST">@csrf @method('DELETE')<button type="submit" class="btn-modal-danger">Eliminar</button></form></div>
                        </div></div>
                    </div>
                @empty
                    <tr class="project-row"><td colspan="4" class="empty-state">No hay generadores registrados por el momento.</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
