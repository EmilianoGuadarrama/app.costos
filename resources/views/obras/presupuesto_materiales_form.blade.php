@extends('layout')
@section('title', 'Agregar Materiales — ' . ($obra->datosDeObra?->nombre ?? 'Obra'))
@section('content')
<style>
...
.pf-ctrl:focus { border-color:#059669; outline:none; }
...
.inp-s:focus { border-color:#059669; outline:none; }
...
.btn-add-row:hover { border-color:#059669; color:#059669; }
...
.btn-guardar { background:#059669; color:#fff; border:none; border-radius:10px; padding:.75rem 1.8rem; font-size:.9rem; font-weight:700; cursor:pointer; }
...
<div class="pf-wrap">
    <a href="{{ route('obras.presupuesto', $obra->id) }}" class="btn-back-sm">
    ...
    <h1 class="pf-title">Agregar Materiales</h1>
    <p class="pf-sub">Obra: <strong>{{ $obra->datosDeObra?->nombre ?? "Obra #$obra->id" }}</strong></p>

    <form action="{{ route('obras.presupuesto.materiales.store', $obra->id) }}" method="POST" id="formMateriales">
        @csrf
        ...
        <!-- Renglones -->
        <div class="pf-card">
            <div class="pf-card-title"><i class="bi bi-list-columns me-1"></i> Renglones de Materiales</div>
        ...
        <div class="form-actions">
            ...
            <button type="submit" class="btn-guardar" id="btn-guardar-materiales">
                <i class="bi bi-check-lg me-1"></i> Guardar Materiales
            </button>
        </div>
    </form>
</div>
...
<script>
// Catálogo de materiales desde PHP → JS
@php
    $catMaterialesMapped = $materiales->map(function($m) {
        return [
            'id'    => $m->id,
            'texto' => $m->nombre . ' (' . ($m->unidadMedida->abreviatura ?? '') . ')',
            'pu'    => (float) $m->costo_actual,
        ];
    });
@endphp
const catMateriales = @json($catMaterialesMapped);

let filaIdx = 0;

function opcionesMateriales() {
    return catMateriales.map(m =>
        `<option value="${m.id}" data-pu="${m.pu}">${m.texto}</option>`
    ).join('');
}
...
function crearFila() {
    ...
    tr.innerHTML = `
        <td>
            <select name="filas[${ri}][id_material]" class="inp-s sel-material" onchange="alCambiarMaterial(this,${ri})">
                <option value="">— Material —</option>
                ${opcionesMateriales()}
            </select>
        </td>
        <td>
            <select name="filas[${ri}][id_bloque]" class="inp-s sel-bloque" style="min-width:90px;">
                <option value="">—</option>
                @foreach($bloques as $b)<option value="{{ $b->id }}">{{ $b->descripcion }}</option>@endforeach
            </select>
        </td>
        <td>
            <select name="filas[${ri}][id_area]" class="inp-s sel-area" style="min-width:90px;">
                <option value="">—</option>
        ...
        <td><input type="number" step="0.01" min="0" name="filas[${ri}][cantidad]" class="inp-s inp-cant" value="1" oninput="recalcFila(${ri})"></td>
        <td><input type="number" step="1" min="0" max="100" name="filas[${ri}][porcentaje_iva]" class="inp-s inp-iva" value="${getIvaGlobal()}" style="width:55px;" oninput="recalcFila(${ri})"></td>
...
function alCambiarMaterial(sel, ri) {
