...
@section('content')
    ...
    <style>
        ...
        @media (max-width: 992px){ .index-panel{ padding:24px; } .project-table{ min-width:1080px; } }
    </style>
    ...
    <div class="dash-index-view">
        <div class="index-panel">
            <div class="header-section">
                <div>
                    <h1>Materiales</h1>
                    <p>Listado general de materiales, marca, unidad y precio.</p>
                </div>
                ...
                <div class="header-actions">
                    ...
                    <a href="{{ Route::has('materiales.create') ? route('materiales.create') : '#' }}" class="btn-add-new">
            ...
            <div class="table-responsive">
                <table class="project-table">
                    <thead>
                    <tr>
                        <th style="width:35%;">Detalle del Material</th>
                        <th>Costo y Marca</th>
                        <th>Unidad</th>
                    ...
                    @forelse(($materiales ?? []) as $material)
                        @php $materialId = $material->id ?? $material->id_material ?? 1; @endphp
                        <tr class="project-row">
                            <td>
                                <div class="title-main">
                                    {{ $material->materiales ?? $material->nombre ?? 'Material sin nombre' }}
                                    <span class="badge-dark-mini">{{ $material->clave ?? 'MAT' }}</span>
                                </div>
                                <div class="desc-text">Marca: {{ $material->marca ?? 'Sin marca registrada' }}</div>
                            </td>
                            <td>
                                <div class="info-stack">
                                    <div><strong>Precio:</strong> {{ $material->precios ?? $material->precio_unitario ?? '0.00' }}</div>
                                    <div><strong>Marca:</strong> {{ $material->marca ?? 'N/D' }}</div>
                                </div>
                            </td>
                            <td>
                                <span class="badge-soft">{{ $material->unidad ?? data_get($material, 'unidadMedida.abreviatura') ?? 'N/D' }}</span>
                            </td>
                            <td class="action-cell">
                                <a href="{{ Route::has('materiales.show') ? route('materiales.show', $materialId) : '#' }}" class="btn-icon-action" title="Ver">
                                    <i class="bi bi-eye"></i>
                                </a>
                                <a href="{{ Route::has('materiales.edit') ? route('materiales.edit', $materialId) : '#' }}" class="btn-icon-action" title="Editar">
                                    <i class="bi bi-pencil-square"></i>
                                </a>
                                @if(Route::has('materiales.destroy'))
                                    <form action="{{ route('materiales.destroy', $materialId) }}" method="POST" style="display:inline;" onsubmit="return confirm('¿Eliminar este material?');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr class="project-row">
                            <td colspan="4" class="empty-state">No hay materiales registrados por el momento.</td>
                        </tr>
                    @endforelse
                    </tbody>
                </table>
