        ...
        <div class="index-panel">
            <div class="header-section">
                <div>
                    ...
                    <p>Listado general de equipos, unidad de uso y costo por hora.</p>
                </div>
                ...
                <div class="header-actions">
                    ...
                    <a href="{{ Route::has('maquinaria_equipo.create') ? route('maquinaria_equipo.create') : '#' }}" class="btn-add-new">
            ...
            <div class="table-responsive">
                <table class="project-table">
                    ...
                    <tbody>
                    @forelse(($maquinarias ?? $maquinaria_equipo ?? []) as $maquinaria)
                        @php $maquinariaId = $maquinaria->id ?? $maquinaria->id_maquinaria ?? 1; @endphp
                        <tr class="project-row">
                            <td>
                                <div class="title-main">
                                    {{ $maquinaria->equipo ?? 'Equipo sin nombre' }}
                                    <span class="badge-dark-mini">{{ $maquinaria->clave ?? 'MQ' }}</span>
                                </div>
                                <div class="desc-text">Registro de maquinaria y equipo para análisis de costo horario.</div>
                            </td>
                            <td>
                                <div class="info-stack">
                                    <div><strong>Costo por hora:</strong> {{ $maquinaria->costo_por_hora ?? '0.00' }}</div>
                                </div>
                            </td>
                            <td>
                                <span class="badge-soft">{{ $maquinaria->unidad ?? data_get($maquinaria, 'unidadMedida.abreviatura') ?? 'N/D' }}</span>
                            </td>
                            <td class="action-cell">
                                <a href="{{ Route::has('maquinaria_equipo.show') ? route('maquinaria_equipo.show', $maquinariaId) : '#' }}" class="btn-icon-action" title="Ver">
                                    <i class="bi bi-eye"></i>
                                </a>
                                <a href="{{ Route::has('maquinaria_equipo.edit') ? route('maquinaria_equipo.edit', $maquinariaId) : '#' }}" class="btn-icon-action" title="Editar">
                                    <i class="bi bi-pencil-square"></i>
                                </a>
                                @if(Route::has('maquinaria_equipo.destroy'))
                                    <form action="{{ route('maquinaria_equipo.destroy', $maquinariaId) }}" method="POST" style="display:inline;" onsubmit="return confirm('¿Eliminar este equipo?');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr class="project-row">
                            <td colspan="4" class="empty-state">No hay maquinaria o equipo registrados.</td>
                        </tr>
                    @endforelse
                    </tbody>
                </table>
