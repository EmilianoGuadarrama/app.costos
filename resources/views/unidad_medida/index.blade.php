...
@section('content')
    ...
    <style>
        ...
        @media (max-width: 992px){ .index-panel{ padding:24px; } .project-table{ min-width:900px; } .detail-grid{ grid-template-columns:1fr; } }
    </style>
        ...
        <div class="index-panel">
            ...
            <div class="header-section">
                ...
                <div class="header-actions">
                    <a href="{{ route('unidad_medida.create') }}" class="btn-add-new"><i class="bi bi-plus-circle me-1"></i> Nueva Unidad</a>
                </div>
            </div>
                ...
                <table class="project-table">
                    ...
                    @forelse($unidades as $unidad)
                        ...
                        <tr class="project-row">
                            ...
                            <td>
                                <div class="info-stack">{{ $unidad->descripcion ?? 'Sin descripción registrada.' }}</div>
                            </td>
                            <td><span class="badge-soft">{{ $unidad->abreviatura }}</span></td>
                            <td class="action-cell">
                                <button type="button" class="btn-icon-action" title="Ver" data-bs-toggle="modal" data-bs-target="#verUnidadModal{{ $unidadId }}"><i class="bi bi-eye"></i></button>
                                <button type="button" class="btn-icon-action" title="Eliminar" data-bs-toggle="modal" data-bs-target="#eliminarUnidadModal{{ $unidadId }}"><i class="bi bi-trash3"></i></button>
                            </td>
                        </tr>

                        <div class="modal fade" id="verUnidadModal{{ $unidadId }}" tabindex="-1" aria-hidden="true">
                            <div class="modal-dialog modal-lg modal-dialog-centered"><div class="modal-content">
                                <div class="modal-header"><h5 class="modal-title">{{ $unidad->nombre }}</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button></div>
                                <div class="modal-body"><div class="detail-grid">
                                    <div class="detail-box"><h6>Información</h6><p><strong>Nombre:</strong> {{ $unidad->nombre }}</p><p><strong>Abreviatura:</strong> {{ $unidad->abreviatura }}</p></div>
                                    <div class="detail-box"><h6>Descripción</h6><p>{{ $unidad->descripcion ?? 'Sin descripción registrada.' }}</p></div>
                                </div></div>
                                <div class="modal-footer"><button type="button" class="btn-modal-light" data-bs-dismiss="modal">Cerrar</button><a href="{{ route('unidad_medida.edit', $unidadId) }}" class="btn-modal-dark">Editar</a></div>
                            </div></div>
                        </div>

                        <div class="modal fade" id="eliminarUnidadModal{{ $unidadId }}" tabindex="-1" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered"><div class="modal-content">
                                <div class="modal-header"><h5 class="modal-title">Eliminar unidad</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button></div>
                                <div class="modal-body">¿Deseas eliminar la unidad <strong>{{ $unidad->nombre }}</strong>?<br><br>Esta acción no se puede deshacer.</div>
                                <div class="modal-footer"><button type="button" class="btn-modal-light" data-bs-dismiss="modal">Cancelar</button><form action="{{ route('unidad_medida.destroy', $unidadId) }}" method="POST">@csrf @method('DELETE')<button type="submit" class="btn-modal-danger">Eliminar</button></form></div>
                            </div></div>
                        </div>
                    @empty
                        <tr class="project-row">
