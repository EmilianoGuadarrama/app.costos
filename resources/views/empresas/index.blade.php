...
<div class="dash-index-view">
    <div class="index-panel">
        ...
        <div class="header-section">
            <div><h1>Empresas</h1><p>Catálogo de empresas registradas en el sistema.</p></div>
            <div class="header-actions"><a href="{{ route('empresas.create') }}" class="btn-add-new"><i class="bi bi-plus-circle me-1"></i> Nueva Empresa</a></div>
        </div>
        <div class="table-responsive">
            <table class="project-table">
                <thead><tr><th>Nombre</th><th>Dirección</th><th>Logo</th><th style="text-align:right;">Acciones</th></tr></thead>
                <tbody>
                @forelse($empresas as $empresa)
                    @php $empresaId = $empresa->id; @endphp
                    <tr class="project-row">
                        <td><strong>{{ $empresa->nombre }}</strong></td>
                        <td>{{ Str::limit($empresa->direccion, 50) ?? 'N/A' }}</td>
                        <td>{{ $empresa->logo_path ?? '—' }}</td>
                        <td class="action-cell">
                            <button type="button" class="btn-icon-action" title="Ver" data-bs-toggle="modal" data-bs-target="#verEmpresaModal{{ $empresaId }}"><i class="bi bi-eye"></i></button>
                            <button type="button" class="btn-icon-action" title="Eliminar" data-bs-toggle="modal" data-bs-target="#eliminarEmpresaModal{{ $empresaId }}"><i class="bi bi-trash3"></i></button>
                        </td>
                    </tr>
                    <div class="modal fade" id="verEmpresaModal{{ $empresaId }}" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog modal-lg modal-dialog-centered"><div class="modal-content">
                            <div class="modal-header"><h5 class="modal-title">{{ $empresa->nombre }}</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button></div>
                            <div class="modal-body"><div class="detail-grid">
                                <div class="detail-box"><h6>Información</h6><p><strong>Nombre:</strong> {{ $empresa->nombre }}</p><p><strong>Dirección:</strong> {{ $empresa->direccion ?? '—' }}</p></div>
                                <div class="detail-box"><h6>Archivos</h6><p><strong>Logo:</strong> {{ $empresa->logo_path ?? 'Sin logo registrado' }}</p></div>
                            </div></div>
                            <div class="modal-footer"><button type="button" class="btn-modal-light" data-bs-dismiss="modal">Cerrar</button><a href="{{ route('empresas.edit', $empresaId) }}" class="btn-modal-dark">Editar</a></div>
                        </div></div>
                    </div>
                    <div class="modal fade" id="eliminarEmpresaModal{{ $empresaId }}" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered"><div class="modal-content">
                            <div class="modal-header"><h5 class="modal-title">Eliminar empresa</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button></div>
                            <div class="modal-body">¿Deseas eliminar la empresa <strong>{{ $empresa->nombre }}</strong>?<br><br>Esta acción no se puede deshacer.</div>
                            <div class="modal-footer"><button type="button" class="btn-modal-light" data-bs-dismiss="modal">Cancelar</button><form action="{{ route('empresas.destroy', $empresaId) }}" method="POST">@csrf @method('DELETE')<button type="submit" class="btn-modal-danger">Eliminar</button></form></div>
                        </div></div>
                    </div>
                @empty
                    <tr class="project-row"><td colspan="4" class="empty-state">No hay empresas registradas por el momento.</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
