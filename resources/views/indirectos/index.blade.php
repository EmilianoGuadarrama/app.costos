...
@section('content')
    <style>
        ...
        .table-custom{width:100%;min-width:850px;margin:0;border-collapse:separate;border-spacing:0}
    ...
    <div class="page-header">
        <div>
            <h2 class="page-title">Indirectos</h2>
            <p class="page-subtitle">Listado general de costos indirectos registrados.</p>
        </div>

        <div class="toolbar">
            <button class="btn btn-soft" type="button"><i class="bi bi-funnel me-1"></i> Filtrar</button>
            <a href="{{ route('indirectos.create') }}" class="btn btn-dark-custom">
                <i class="bi bi-plus-circle me-1"></i> Nuevo Indirecto
            </a>
        </div>
    </div>
        ...
        <div class="table-wrap">
            <table class="table-custom">
                ...
                @forelse(($indirectos ?? []) as $indirecto)
                    <tr>
                        <td>{{ $indirecto->clave ?? '' }}</td>
                        <td>{{ $indirecto->concepto ?? '' }}</td>
                        <td>{{ $indirecto->porcentaje ?? '' }}</td>
                        <td>{{ $indirecto->importe ?? '' }}</td>
                        <td>
                            <div class="action-group">
                                <a class="action-btn view" href="{{ Route::has('indirectos.show') ? route('indirectos.show', $indirecto->id ?? 1) : '#' }}" title="Ver"><i class="bi bi-eye"></i></a>
                                <a class="action-btn edit" href="{{ route('indirectos.edit', $indirecto->id ?? 1) }}" title="Editar"><i class="bi bi-pencil"></i></a>
                                <form action="{{ Route::has('indirectos.destroy') ? route('indirectos.destroy', $indirecto->id ?? 1) : '#' }}" method="POST" onsubmit="return confirm('¿Deseas eliminar este indirecto?');" style="display:inline-block;">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    @for($i = 1; $i <= 6; $i++)
                        <tr>
                            ...
                            <td>
                                <div class="action-group">
                                    <a class="action-btn view" href="{{ Route::has('indirectos.show') ? route('indirectos.show', $i) : '#' }}" title="Ver"><i class="bi bi-eye"></i></a>
                                    <a class="action-btn edit" href="{{ route('indirectos.edit', $i) }}" title="Editar"><i class="bi bi-pencil"></i></a>
